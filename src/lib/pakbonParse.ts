...
// na de prijs aan het eind van de regel staat. De grens rond elk getal is bewust op
// \w (niet \d): anders telt bv. de "44" in "IP44" of de "6" in "6W" ook mee als los
// getal, terwijl die vastzitten aan een letter en dus duidelijk geen aparte aantal-kolom
// zijn. Bij twijfel valt het aantal terug op 1 - de gebruiker controleert en corrigeert
// de gevonden regels altijd nog voordat ze toegepast worden.
export function parsePakbonText(text: string): PakbonLineMatch[] {
  const byEan = new Map<string, number>();

  for (const rawLine of text.split(/\r?\n/)) {
    const eanMatch = rawLine.match(/(?<!\w)(\d{13}|\d{8})(?!\w)/);
    ...
    const qtyMatches = withoutEan.match(/(?<!\w)\d{1,5}(?!\w)/g);
...
function extractBoxCount(line: string, unitsPerBox: number): number {
  ...
  const numbers: string[] = withoutPrices.match(/(?<!\w)\d{1,6}(?!\w)/g) ?? [];
